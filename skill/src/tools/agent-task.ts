...
export const xiaozhiAgentTaskTool = (client: XiaozhiWebSocketClient, api: OpenClawPluginApi) => ({
  ...
  async execute(_id: string, params: Record<string, unknown>) {
    ...
    const { action, taskId, prompt, device_id } = params as {
      action: string; taskId?: string; prompt?: string; device_id?: string;
    };
    if (!action) throw new Error("Action is required");
    if ((action === "status" || action === "cancel") && !taskId) throw new Error(`taskId is required for ${action}`);
    if (action === "execute" && !prompt) throw new Error("prompt is required for execute");
    ...
    if (device_id) args.device_id = device_id;

...
let client: XiaozhiWebSocketClient | null = null;

function loadConfig(api: OpenClawPluginApi) {
...
export async function startPlugin(api: OpenClawPluginApi): Promise<void> {
  if (client) { api.logger.warn("xiaozhi: Plugin already started"); return; }
...
export function getClient(): XiaozhiWebSocketClient | null { return client; }
